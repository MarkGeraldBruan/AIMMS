...
class RpcSemiHighExport implements FromArray, WithEvents
{
    protected $request;
    ...
    public function array(): array
    {
        ...
        $data[] = ['REPORT ON THE PHYSICAL COUNT OF SEMI-EXPENDABLE PROPERTIES (HIGH VALUE)', '', '', '', '', '', '', '', '', '', '', '', '', '']; // Row 1 - Title
        $data[] = [$formattedDate ? 'As of ' . $formattedDate : '', '', '', '', '', '', '', '', '', '', '', '', '', '']; // Row 2 - Date
        $data[] = ['Properties with unit value of ₱50,000.00 and above', '', '', '', '', '', '', '', '', '', '', '', '', '']; // Row 3 - Value info
        if (!empty($appliedFilters)) {
            $data[] = [$appliedFilters, '', '', '', '', '', '', '', '', '', '', '', '', '']; // Row 4 - Applied Filters
            $data[] = ['', '', '', '', '', '', '', '', '', '', '', '', '', '']; // Empty row (Row 5)
        } else {
            $data[] = ['', '', '', '', '', '', '', '', '', '', '', '', '', '']; // Empty row (Row 4)
        }

        // Header grid layout - exactly matching screen view (4 columns)
        $data[] = ['Entity Name:', $entityName, '', '', 'Accountable Officer:', $accountablePerson, '', '', 'Position:', $position, '', '', 'Office:', $office]; // Row 5/6
        $data[] = ['', '', '', '', '(Name)', '', '', '', '(Designation)', '', '', '', '(Station)', '']; // Row 6/7
        $data[] = ['', '', '', '', '', '', '', '', '', '', '', '', 'Fund Cluster:', $fundCluster]; // Row 7/8
        $data[] = ['', '', '', '', '', '', '', '', '', '', '', '', '', '']; // Empty row (Row 8/9)
        ...
        $data[] = [$accountabilityText, '', '', '', '', '', '', '', '', '', '', '', '', '']; // Row 9/10
        $data[] = ['', '', '', '', '', '', '', '', '', '', '', '', '', '']; // Empty row (Row 10/11)
        ...
        // Table rows - matching screen view structure
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Row 1: Title - Center and bold
                $sheet->mergeCells('A1:N1');
                $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
                $sheet->getStyle('A1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle('A1')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);

                // Row 2: As of date - Center and bold
                $sheet->mergeCells('A2:N2');
                $sheet->getStyle('A2')->getFont()->setBold(true)->setSize(12);
                $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

                // Row 3: Value info - Center and italic
                ...
                // Add borders around header fields
                ...
                // Set proper alignment for data cells
